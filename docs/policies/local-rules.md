...
- Resumed phase work proceeds one phase of the plan linked from `docs/work/0_japan-street-scene/plan.md` at a time. The implementer reports to the integration owner at the end of a phase and waits; it does not start the next phase on its own. The integration owner maintains the current work entry; the original manager-authored plan at `docs/work/0_japan-street-scene/historical/PLAN.md` stays unchanged.
...
- A tool that can be run on its own guards its main block with the `isMainModule()` in `tools/serve.js` (`resolve(process.argv[1])` against `fileURLToPath(import.meta.url)`), never with a hand-built `file:///` string: the hand-built form is true on Windows and false on every POSIX argv[1], so it passes review here and skips the whole tool on CI. A gate added to `tools/test.js` brings its evidence marker with it, in `GATES`, on the tool's own success path after the work.
