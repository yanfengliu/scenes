...
Recorded rather than left to be discovered, in the order a next pass should take them.

1. **The camera's eye height is 9.1 m above the north lawn.** The photograph's four measured landmarks fix the products `tanH * d` and `tanV * d`, not `d`, and the file's own EXIF lens then settles the distance — and the eye height follows with no freedom left. It is built as terrain (the lawn falls 7.5 m from the building to the camera's station), which reproduces the photograph's framing exactly and is what a photographer standing below a terrace would see. The record does not support a 7.5 m drop over that run, so this is the frame's reading and not the survey's. Changing it moves the wall base off the row the photograph measures.
2. **The portico's pediment apex is 82 px below the photograph's row.** Closing it needs a 4.19 m apex that would stand above the balustrade, hide the parapet and fill sky the photograph has. The silhouette was chosen over the row, deliberately.
3. **The south front is bare below the balustrade**: windows but no pediments, no belt course, no ground-floor rustication. Nothing scored can see it; the orbit frames can.
4. **The roofscape is blockout**: a flat deck, a ridge, chimneys and a flag. The balustrade hides it from the photo view.
5. **The fountain is three cylinders and two spheres**, and the fence's pickets are a uniform box array.
6. **`compare`'s overlay is scene 1's for every scene** — it draws `src/layout.js`'s landmark boxes, so `out/wh/overlay.png` labels the White House with "cherry" and "mountains". Annotation only, it moves no score, and the correct one for this scene is `out/wh/overlay-wh.png` from `out/wh/scratch/ov.mjs`.
7. **The scene reads 0.1450 cell distance against scene 1's 0.0596.** The two are not comparable subjects (see `scores.md`), but the facade's tonal separation, the portico's depth from an orbit and the grounds' flatness are where the next gains are, in that order.
