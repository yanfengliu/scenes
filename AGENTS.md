...
- `npm test` runs `shot`, `compare` and `placement`, then asserts the scores against the thresholds in `docs/PLAN-scores.md`. Needs network for the Three.js CDN and a cached Playwright chromium (`npx playwright install chromium` once).
- `npm run placement` casts the photo camera's ray through each entry of `PLACEMENT_CHECKS` in `src/layout.js` and fails unless the first mesh hit is the named one: the check for what the scores cannot see (a piece built inside another, a canopy hiding a missing trunk).
